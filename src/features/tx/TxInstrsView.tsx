...
function InfoRow({ label, value, valueColor = "text-gray-200" }: { label: string; value: string | number; valueColor?: string }) {
    return (
            ...
            <div className="rounded-lg bg-gray-800/50 p-4 space-y-2">
                ...
                <div className="flex items-center justify-between mb-3">
                    <span className="text-gray-400 text-sm">
                        Instruction <span className="text-white font-semibold">{instr.number}</span> of {instr.total}
                    </span>
                ...
                {instr.callData && (
                    <div className="mt-3">
                        ...
                        <p className="font-mono text-xs text-gray-300 break-all leading-relaxed mt-1">
                            {instr.callData}
                        </p>
                    </div>
                )}
                ...
                <div className="mt-4">
                    ...
                    <div className="rounded-lg border border-gray-700 overflow-hidden">
                        {instr.memoryMappings.map((m, i) => (
                            <div
                                key={i}
                                className={`flex items-center justify-between px-3 py-2 text-xs font-mono ${i % 2 === 0 ? 'bg-gray-800/30' : 'bg-gray-800/50'
                                    }`}
                            >
                                <span className="text-gray-400">{m.range}</span>
            ...
            {instr.lastConditionalJump && (
                <div className="mt-4">
                    ...
                    <div className="flex items-center gap-2 text-xs font-mono">
                        ...
                        <span className="text-gray-400">PC = {instr.lastConditionalJump.pc}</span>
                        <span className="text-gray-500">|</span>
                        <span className="text-gray-400">Condition = {instr.lastConditionalJump.condition}</span>
                    </div>
                </div>
            )}
        </div>
    ...
    return (
        <div className={`rounded-xl border border-gray-700 bg-gray-900 ${className}`}>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700">
                <h3 className="text-lg font-bold text-white">TX_INSTRS</h3>
            ...
            <div className="px-5 divide-y divide-gray-800 max-h-[500px] overflow-y-auto">
